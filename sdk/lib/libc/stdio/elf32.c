...
bool elf_checkFile32(Elf32_Ehdr *elf)
{
    return (
               elf->e_ident[0] == '\177' &&
               elf->e_ident[1] == 'E'    &&
...
v_region_t elf_getMemoryBounds32(Elf32_Ehdr *elf)
{
    v_region_t  elf_reg;
    vaddr_t     sect_start;
    vaddr_t     sect_end;
...
void elf_load32(Elf32_Ehdr *elf, uint32_t offset)
{
    paddr_t     src;
    paddr_t     dst;
    uint64_t    len;
...
Elf32_Shdr * getSHStrTab32(Elf32_Ehdr * ehdr)
{
    ...
    // xprintf("[%s:%d] Elf32_Shdr size:0x%p\n",__func__,__LINE__, sizeof(Elf32_Shdr));
    for (i = 0; i < shnum; i++, shdr++)
    {
     //   xprintf("[%s:%d] shdr[%d]->sh_type:%d\n",__func__,__LINE__,i,shdr->sh_type);
        if (shdr->sh_type == SHT_STRTAB)
        {
            ...
            {
                // xprintf("[%s:%d] found nameStr:%s shdr:0x%p\n",__func__,__LINE__,
                // nameStr, shdr);
                return shdr;
            }
        }
...
Elf32_Shdr * getSHdrByName32(Elf32_Ehdr * ehdr, const char * name, Elf32_Word sh_type)
{
  ...
  //  xprintf("[%s:%d] ehdr->e_shoff:%d\n",__func__,__LINE__,ehdr->e_shoff);
    ...
    // xprintf("[%s:%d] shstrtabhdr:0x%p\n",__func__,__LINE__, shstrtabhdr);
    if (shstrtabhdr == NULL)
    {
...
int parseSegment32(Elf32_Ehdr * ehdr)
{
    ...
    for (i = 0; i < ehdr->e_phnum; i++, phdr++)
    {
        ...
        for (j = 1; j < ehdr->e_shnum; j++, shdr++)
        {
            ...
            /* ��ַ����ƫ����segment��ַ��Χ�ڵ�section���鵽��segment */
...
void *__vdsosym32( const char *name)
{
	size_t i = 0;
    int  sym_cnt = 0;
    ...
    shdrSymTab = getSHdrByName32(ehdr, SECTION_NAME_SYMTAB, SHT_SYMTAB);
    if (shdrSymTab == NULL)
    {
        xprintf("\n [%s:%d] getSymTab faild! shdrSymTab:0x%p\n",__func__, __LINE__,shdrSymTab);
    //    xprintf("$$$$$$$$ [%s:%d] eh->e_phoff:0x%p  \n",__func__,__LINE__,ehdr->e_phoff );
        return NULL;
    }
    shdrStrTab = getSHdrByName32(ehdr, SECTION_NAME_STRTAB, SHT_STRTAB);
    if (shdrStrTab == NULL)
    {
        xprintf(" [%s:%d]  getStrTab faild\n",__func__, __LINE__);
        return NULL;
    }
   ...
   // xprintf("\n[%s:%d] \n",__func__,__LINE__ );
    ...
    for (i = 0; i < sym_cnt; i++, sym++)
    {
      ...
      //  xprintf("%s", getStrTabStr(ehdr, shdrStrTab, sym->st_name));
        if (strcmp(name, getStrTabStr32(ehdr, shdrStrTab, sym->st_name))==0){
...
size_t LocVdsoStart32(size_t vdsoStart, const char *elfHead, const size_t len)
{
    char *head = NULL;
    s32 i;
    s32 loop;
