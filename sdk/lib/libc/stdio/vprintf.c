...
static void print_uint(struct vprintf_context *ctx, size_t n, int base,
                       char pad, int width) {
    char tmp[32];
    int i = sizeof(tmp) - 2;
    do {
...
void vprintf_with_context(struct vprintf_context *ctx, const char *fmt,
                          va_list vargs) {
#ifdef VPRINTF_CONTEXT_BUFSIZE
    char ctx_buf[VPRINTF_CONTEXT_BUFSIZE] = {0};

    if (!ctx->buf)
    {
        ctx->buf = ctx_buf;
        ctx->size = VPRINTF_CONTEXT_BUFSIZE;
        ctx->index = 0;
    }
#endif
    while (*fmt) {
        if (*fmt != '%') {
            ...
            char pad = ' ';
            bool alt = false;
            uintmax_t n;		//	This value should be uintmax_t value, as 32-bits platform support uint64_t also
            bool not_attr = false;
            while (*++fmt) {
                switch (*fmt) {
                    case 'l':
...
static void snprintf_printchar(struct vprintf_context *ctx, char ch) {
    if (ctx->size && ctx->index == ctx->size) {
        // The buffer is full.
        return;
    }
